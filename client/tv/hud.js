...
window.HUD = {
  ...
  // ── Dialogue overlay — delegated to dialogue-hud.js (window.DialogueHUD) ──
  showDialogue(scene, npcName, text) { DialogueHUD.showDialogue(scene, npcName, text); },
  hideDialogue(scene) { DialogueHUD.hideDialogue(scene); },
  _forceDestroyDialogue() { DialogueHUD._forceDestroyDialogue(); },
  ...
  // ── Victory Screen — delegated to victory.js (window.Victory) ──
  showVictoryScreen(scene, data) { Victory.showVictoryScreen(scene, data); },
  _destroyVictoryScreen() { Victory._destroyVictoryScreen(); },
  ...
  shutdown() {
    ...
    this.questAnnouncementQueue = [];
    this.questAnnouncementActive = false;
    // Clean up floating damage/heal texts
    if (this.damageTexts) {
      this.damageTexts.forEach(dt => {
        if (dt && dt.text && dt.text.destroy) dt.text.destroy();
      });
      this.damageTexts = [];
    }
    // Clean up boss HP bar objects
    if (this.bossBar) {
      ['bg', 'fill', 'border', 'nameText', 'hpText'].forEach(key => {
        if (this.bossBar[key] && this.bossBar[key].destroy) {
          this.bossBar[key].destroy();
        }
      });
      this.bossBar = null;
    }
